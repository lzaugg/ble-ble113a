...
function Messenger(hardware, options) {

  this.bgLib = new bglib();
  this.configuration = {
    connectionSupervisionTimeout: 10, //10 - 3200 (in units of 10ms)
    connectionIntervalMax:10, //6 - 3200 (in units of 1.25ms)
    connectionIntervalMin:6 //6 - 3200 (in units of 1.25ms)
  };
  extend(this.configuration, options);
  ...
  this.commandPacketTimeoutDuration = 500;
...
Messenger.prototype.debug = function() {
  if (this.configuration.log) {
    this.configuration.log.debug(arguments);
  } else if (DEBUG) {
    console.log(arguments);
  }
}

Messenger.prototype.warn = function() {
  if (this.configuration.log) {
    this.configuration.log.warn(arguments);
  } else {
...
Messenger.prototype.reset = function(callback) {
  ...
  this.stopScanning();
  this.bootSequence(connectTimeout, callback);
...
Messenger.prototype.systemReset = function(callback) {
  this.execute(bglib.api.systemReset, [0], callback);
}
...
Messenger.prototype.connect = function(address, addressType, callback) {
  ...
  //this.execute(bglib.api.gapConnectDirect, [address, addressType, 7.5, 20, 1000, 0], callback);
  // conn_interval_min: range: 6 - 3200 (in units of 1.25ms)
  // conn_interval_max: range: 6 - 3200 (in units of 1.25ms)
  // timeout: range: 10 - 3200 (in units of 10ms)
  var connIntervalMin = this.configuration.connectionIntervalMin;
  var connIntervalMax = this.configuration.connectionIntervalMax;
  var timeout = this.configuration.connectionSupervisionTimeout;
  this.execute(bglib.api.gapConnectDirect, [address, addressType, connIntervalMin, connIntervalMax, timeout, 0], callback);
}
  ...
  this.bgLib.getPacket(command, params, function(err, packet) {
    ...
    // Else we're going to send it down the wire
      ...
      packet.getByteArray(function(bArray) {
          ...
          self.debug("Byte Array to be sent: ", bArray);
...
Messenger.prototype.parseIncomingPackets = function(data) {
  ...
  var incomingPackets = this.bgLib.parseIncoming(data, function callbackSeeker(err, parsedPackets) {
    ...
    for (var i = 0; i < parsedPackets_length; i++) {
      ...
      self.debug("Got this packet: ", parsedPacket);
      ...
      switch (parsedPacket.packet.mType & 0x80) {
          ...
          case 0x00:
              ...
              break;
          default:
                  self.warn("Malformed packet returned...");
      }
    }
  });
}
